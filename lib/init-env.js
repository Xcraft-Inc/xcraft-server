...
const initEtc = (configPath, projectPath, appId = null) => {
  ...
  if (!initialized) {
    ...
    [/^xcraft-(core|contrib)-/, /^goblin-/].forEach((filter) =>
      Etc.createAll(nodeModulesDir, filter, overriderFile, appId)
    );
  }
