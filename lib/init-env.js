...
function isInitialized (initializedFlagFile, xcraftConfigFilePath) {
  try {
    ...
    return init[xcraftConfigFilePath] === fs.statSync (xcraftConfigFilePath).mtime;
  } catch (ex) {
    return false;
  }
}
...
module.exports = (configPath, projectPath) => {
  ...
  if (!initialized) {
    require ('xcraft-core-etc') ().createAll (nodeModulesDir, /^xcraft-(core|contrib)/, overriderFile);
  }
